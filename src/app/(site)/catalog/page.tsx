...
export default async function CatalogPage() {
  ...
  const heroImage = gallery[0]?.imageUrl || catalogueCategories[1].image;
  ...
  return (
    <main>
      ...
      <section className="surface-grid bg-[#f4f4f1] py-24"><div className="container-shell"><SectionHeading eyebrow="Catalog overview" title="Product Categories for Every Storage Need" /><Stagger className="mt-10 grid gap-px bg-zinc-300 sm:grid-cols-2 lg:grid-cols-3">{catalogueCategories.map((category) => <StaggerItem key={category.slug} className="h-full"><Link href={`/products/${category.slug}`} className="group flex h-full flex-col bg-white p-7 hover:bg-zinc-950 hover:text-white"><h3 className="text-xl font-semibold">{category.name}</h3><p className="mt-2 text-sm text-zinc-500 group-hover:text-zinc-400">{category.productCount} products</p><ArrowRight size={17} className="mt-8 text-red-600 transition-transform group-hover:translate-x-1" /></Link></StaggerItem>)}</Stagger></div></section>
      <section className="py-24"><div className="container-shell"><SectionHeading eyebrow="Explore online" title="See Full Details on Our Product Pages" /><div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-4">{catalogueProducts.slice(0, 4).map((product) => <CatalogueProductCard key={product.id} product={product} />)}</div></div></section>
