...
export default async function HomePage() {
  ...
  const whyEnvironments: WhyEnvironment[] = [
    {
      slug: "heavy-duty-pallet-racking",
      title: "Pallet Racking",
      location: "Heavy-Duty Storage",
    },
  ...
  ].flatMap(({ slug, title, location }) => {
    const product = homeProducts.find((item) => item.slug === slug);
    return product?.image ? [{ title, location, href: product.href, image: product.image }] : [];
  });
...
{manufacturing&&<section className="bg-zinc-950 text-white"><div className="grid min-h-[610px] lg:grid-cols-2"><div className="relative min-h-[420px]"><SmartImage src={String(mc.image||"")} alt="Storage system installation and coordination" fill className="object-cover opacity-80" sizes="50vw"/></div><div className="dark-grid flex items-center p-8 sm:p-14 lg:p-20"><div><h2 className="mt-5 text-balance section-heading text-white">{manufacturing.title}</h2><p className="section-description mt-5 text-zinc-400">{manufacturing.subtitle}</p><Link href="/contact" className="btn-light mt-8">{String(mc.cta||"Discuss your requirement")} <ArrowRight size={16}/></Link></div></div></div></section>}
{testimonials.length>0&&<section className="bg-[#f4f4f1] py-20"><div className="container-shell"><SectionHeading eyebrow="Client feedback" title="What Our Clients Say"/><div className="mt-10 grid gap-4 md:grid-cols-2">{testimonials.map(item=><blockquote key={item.id} className="bg-white p-8"><p className="text-lg leading-8">“{item.content}”</p><footer className="mt-6 text-sm font-semibold">{item.clientName}{item.company&&<span className="block text-xs font-normal text-zinc-500">{item.company}</span>}</footer></blockquote>)}</div></div></section>}
{gallery.length>0&&<section className="py-24"><div className="container-shell"><div className="flex items-end justify-between gap-6"><SectionHeading compact title="See Our Work"/><Link className="btn-secondary hidden sm:inline-flex" href="/gallery">View Gallery</Link></div><div className="mt-10 grid auto-rows-[220px] gap-4 md:grid-cols-4">{gallery.slice(0,5).map((item,i)=><Link href="/gallery" key={item.id} className={`group relative overflow-hidden ${i===0?"md:col-span-2 md:row-span-2":""}`}><SmartImage src={item.imageUrl} alt={item.altText} fill className="object-cover transition duration-700 group-hover:scale-105" sizes={i===0?"50vw":"25vw"}/><span className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent"/><strong className="absolute bottom-4 left-4 text-sm text-white">{item.title}</strong></Link>)}</div></div></section>}
{posts.length>0&&<section className="bg-[#f4f4f1] py-24"><div className="container-shell"><SectionHeading compact title="Tips for Smarter Storage"/><div className="mt-10 grid gap-5 md:grid-cols-2">{posts.slice(0,2).map(({post,category})=><Link href={`/blog/${post.slug}`} key={post.id} className="group grid overflow-hidden bg-white sm:grid-cols-[220px_1fr]"><div className="relative min-h-52"><SmartImage src={post.featuredImage||""} alt={post.title} fill className="object-cover transition duration-700 group-hover:scale-105" sizes="220px"/></div><div className="p-6"><p className="text-[.62rem] font-bold uppercase tracking-widest text-red-600">{category?.name||"Insight"}</p><h3 className="card-title mt-3 transition-colors group-hover:text-red-600">{post.title}</h3><p className="mt-3 text-sm leading-6 text-zinc-500">{post.excerpt}</p></div></Link>)}</div></div></section>}
{cta&&<section className="relative isolate overflow-hidden bg-red-700 py-20 text-white"><div aria-hidden className="absolute inset-0 -z-20"><SmartImage src="/Footer%20imagesbg.png" alt="" fill sizes="100vw" className="object-cover object-[68%_50%]"/></div><div aria-hidden className="absolute inset-0 -z-10 bg-[linear-gradient(90deg,rgba(58,7,12,.9)_0%,rgba(58,7,12,.88)_42%,rgba(58,7,12,.74)_64%,rgba(58,7,12,.52)_82%,rgba(58,7,12,.46)_100%)]"/><div aria-hidden className="absolute inset-0 -z-10 bg-red-950/45 lg:hidden"/><div className="container-shell flex flex-col justify-between gap-9 lg:flex-row lg:items-end"><div className="max-w-4xl"><h2 className="section-heading mt-4 text-balance">{cta.title}</h2><p className="section-description mt-5 text-red-100">{cta.subtitle}</p></div><Link href="/request-a-quote" className="btn-light shrink-0">Request a Quote <ArrowRight size={17}/></Link></div></section>}
</main>}
