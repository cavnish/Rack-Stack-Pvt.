...
export function CatalogDownload({ available, gated }: { available: boolean; gated: boolean }) {
  ...
  async function requestDownload(data: Record<string, string>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error || "Catalog download is unavailable.");
      setState("done");
  ...
  if (!available) return <div className="border border-amber-200 bg-amber-50 p-5 text-sm text-amber-900"><strong>Catalog update in progress.</strong><p className="mt-1 text-amber-800">Contact our team for the latest product information while the downloadable catalog is being prepared.</p></div>;
  ...
  return <form onSubmit={(event)=>{event.preventDefault();requestDownload(Object.fromEntries(new FormData(event.currentTarget)) as Record<string,string>)}} className="grid gap-4 sm:grid-cols-2">
    ...
    {state==="done"&&<p className="flex items-center gap-2 text-sm text-green-700 sm:col-span-2"><CheckCircle2 size={16}/>Download recorded. The catalog opened in a new tab.</p>}
    ...
    <p className="text-[.68rem] leading-5 text-zinc-500 sm:col-span-2">We use these details only to provide the catalog and support relevant follow-up. See our privacy policy.</p>
  </form>;
}
