...
export type ProductCardProduct = Product & Partial<Pick<CatalogueProduct, "keySpec" | "application" | "applicationTitle">>;
...
export function ProductCard({ product, index = 0 }: { product: ProductCardProduct; index?: number }) {
  ...
  return (
    <div className="group flex min-h-[520px] flex-col overflow-hidden border border-zinc-200 bg-white transition-colors duration-300 hover:border-zinc-800">
      <Link href={`/products/${product.slug}`} className="relative block aspect-[4/3] overflow-hidden bg-zinc-200">
        {image ? (
          <SmartImage src={optimizeImage(image, 760)} alt={product.name} fill className="object-cover transition duration-700 group-hover:scale-105" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" />
        ) : null}
